...
CONFIG_FILE = BASE_DIR / "duifenyi.ini"
...
def get_coords(course_name):
    try:
        if LOCATION_FILE.exists():
            ...
            if course_name in locs:
                c = locs[course_name]
                return c.get("lng",""), c.get("lat","")
    except: pass
    return "", ""
...
class Auth:
    ...
    def load():
        ...
        try:
            ...
            ck = config.get("INFO","cookie",fallback="")
...
class CI:
    ...
    def mark_done(cls, rid):
        if rid and rid not in cls.done:
            cls.done.append(rid)
    ...
    def pend(cls, rows):
        ...
        for r in rows:
            ...
            # Already signed: status filled, or date filled, or studentid filled
    ...
    def code(cls, uid, code):
        try:
            ...
            if r.status_code == 200:
                try:
                    msg = r.json().get("msgbox",""); log.i(f"签到: {msg}")
                    return "成功" in msg
                except: return False
            return False
        except: return False
    ...
    def qr(cls, ci):
        try:
            ...
            for eid in ["DivOK","divok","divOK"]:
                e = soup.find(id=eid)
                if e and "签到成功" in e.get_text(): return True
            return "签到成功" in soup.get_text()
        except: return False
    ...
    def loc(cls, uid, lng, lat, cid, tcid):
        try:
            ...
            session.headers["User-Agent"] = "Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.50(0x18003234) NetType/WIFI Language/zh_CN"
            h = {"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
                 "Referer": HOST+"/_CheckIn/MB/CheckInStudent.aspx?moduleid=16&pasd=",
    ...
    def lc(cls, ci, cid):
        urls = [
            f"{HOST}/_CheckIn/MB/TeachCheckIn.aspx?classid={cid}&temps=0&checktype=3&isrefresh=0&timeinterval=0&roomid=0&match=",
            f"{HOST}/_CheckIn/MB/CheckInStudent.aspx?moduleid=16&pasd=&checkinid={ci}",
        ]
        for url in urls:
            try:
...
class Loop:
    ...
    def start(s, ci, cn, rt, cid="", delay=0): s.r=True; s.ci=ci; s.cn=cn; s._rt=rt; s._cid=cid; s._delay=delay; s._pending=None; s._tick=0; CI.rst(); log.i(f"监听【{cn}】签到, 延迟{delay}秒"); s._poll()
        ...
        try:
            ...
            if rows is None:
                ...
                if p is None:
                    ...
                    CI.mark_done(cid)  # Mark before attempt to prevent retry
                    if s._delay > 0 and not s._pending:
                        s._pending = (ct, p)
                        log.i(f"将在 {s._delay} 秒后自动签到...")
                        if s._rt: s._rt.after(s._delay * 1000, s._do_pending)
                    else:
                        if s._do(ct,p): log.ok(f"签到成功! ID:{cid}")
                        else: log.w(f"签到失败! ID:{cid}")
    ...
    def _do(s, ct, info):
        uid = Course.uid()
        if not uid: log.e("无用户ID"); return False
        if ct == "1":
            ...
            if not lng or not lat:
                lng, lat = get_coords(s.cn)
                if not lng:
                    lng, lat = "114.39437", "22.70462"
                    log.w(f"未配置【{s.cn}】坐标，使用默认。请在locations.json添加")
            return CI.loc(uid, lng, lat, s._cid, s.ci)
        else: log.w(f"未知类型: {ct}")
        return False
...
class App:
    def __init__(s, rt):
        s.rt = rt; s.rt.title("对分易签到助手"); s.rt.configure(bg=CB); s.rt.resizable(True, True); s.rt.minsize(960, 680)
    ...
    def _ui(s):
        ...
        s._sl = tk.Label(hd, textvariable=s._sv, font=("Microsoft YaHei UI",9), bg=CPRI, fg="#a0c4ff"); s._sl.pack(side=tk.RIGHT, padx=20, pady=10)
        ...
        tk.Label(lc, text="账号登录", font=("Microsoft YaHei UI",12,"bold"), bg=CC, fg=CTX).pack(anchor=tk.W)
        ...
        tk.Label(wf, text="支持数字码 + 二维码 + 定位签到", font=("Microsoft YaHei UI",8), bg=CC, fg=CT2).pack()
        s._le = tk.Entry(wf, font=("Microsoft YaHei UI",10), relief=tk.FLAT, bg="#f1f2f6", insertbackground=CPRI); s._le.pack(fill=tk.X, pady=(8,4), ipady=6)
        ...
        tk.Button(wf, text="微信登录", command=s._wx, font=("Microsoft YaHei UI",10,"bold"), bg=CWE, fg="white", relief=tk.FLAT, cursor="hand2", activebackground="#06ad56", activeforeground="white", padx=24, pady=6).pack(pady=4)
        tk.Label(wf, text="电脑微信打开链接 -> 右上角复制 -> 粘贴到此处", font=("Microsoft YaHei UI",9), bg=CC, fg=CT2).pack(pady=(4,0))
        ...
        ht = tk.Text(wf, font=("Microsoft YaHei UI",9), bg=CC, fg=CT2, relief=tk.FLAT,
                     height=7, wrap=tk.WORD, borderwidth=0, cursor="arrow")
        ht.insert("1.0", help_text); ht.configure(state=tk.DISABLED)
        ht.pack(fill=tk.X, pady=(8,0))

        pf = tk.Frame(t2, bg=CC); pf.pack(fill=tk.X, padx=6, pady=(10,4))
        tk.Label(pf, text="不支持二维码签到", font=("Microsoft YaHei UI",8), bg=CC, fg=CDAN).pack()
        for lb, vn, sh in [("账号","_ue",""),("密码","_pe","*")]:
            tk.Label(pf, text=lb, font=("Microsoft YaHei UI",9,"bold"), bg=CC, fg=CTX).pack(anchor=tk.W, pady=(8,2))
            en = tk.Entry(pf, font=("Microsoft YaHei UI",10), show=sh, relief=tk.FLAT, bg="#f1f2f6", insertbackground=CPRI); en.pack(fill=tk.X, ipady=6); setattr(s, vn, en)
        tk.Button(pf, text="登录", command=s._pwd, font=("Microsoft YaHei UI",10,"bold"), bg=CPRI, fg="white", relief=tk.FLAT, cursor="hand2", activebackground="#2751c0", activeforeground="white", padx=24, pady=6).pack(pady=(10,4))

        df = tk.Frame(lc, bg=CC); df.pack(fill=tk.X, pady=(10,0))
        tk.Label(df, text="签到延迟（检测到后等待N秒再签，0=立即）", font=("Microsoft YaHei UI",8), bg=CC, fg=CT2).pack(anchor=tk.W)
        s._se = tk.Entry(df, font=("Microsoft YaHei UI",10), relief=tk.FLAT, bg="#f1f2f6", width=6, insertbackground=CPRI); s._se.insert(0,"0"); s._se.pack(anchor=tk.W, ipady=4)

        cc = tk.Frame(lf, bg=CC, highlightbackground=CBO, highlightthickness=1, padx=16, pady=14); cc.pack(fill=tk.X, pady=(12,0))
        tk.Label(cc, text="课程选择", font=("Microsoft YaHei UI",12,"bold"), bg=CC, fg=CTX).pack(anchor=tk.W)
        s._cv = tk.StringVar(); s._co = ttk.Combobox(cc, textvariable=s._cv, state="readonly", font=("Microsoft YaHei UI",10)); s._co.pack(fill=tk.X, pady=(10,8), ipady=2)
        br = tk.Frame(cc, bg=CC); br.pack(fill=tk.X)
        s._sb = tk.Button(br, text="▶  开始监听", command=s._go, font=("Microsoft YaHei UI",11,"bold"), bg=CSUC, fg="white", relief=tk.FLAT, state=tk.DISABLED, cursor="hand2", activebackground="#1aa65a", activeforeground="white", padx=16, pady=8); s._sb.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,4))
        s._xb = tk.Button(br, text="■  停止", command=s._stop, font=("Microsoft YaHei UI",11,"bold"), bg=CDAN, fg="white", relief=tk.FLAT, state=tk.DISABLED, cursor="hand2", activebackground="#d1344f", activeforeground="white", padx=16, pady=8); s._xb.pack(side=tk.RIGHT, fill=tk.X, expand=True, padx=(4,0))
        ...
        tk.Label(rh, text="运行日志", font=("Microsoft YaHei UI",12,"bold"), bg=CC, fg=CTX).pack(side=tk.LEFT)
        tk.Button(rh, text="清空", command=lambda: s._lb.delete("1.0",tk.END), font=("Microsoft YaHei UI",8), bg=CBO, fg=CTX, relief=tk.FLAT, cursor="hand2", padx=10, pady=2).pack(side=tk.RIGHT)
